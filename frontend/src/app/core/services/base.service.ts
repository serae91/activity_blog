...
export class BaseService {
  ...
  protected post<T>(url: string, body: any, errorMessage: (error: HttpErrorResponse) => string, successMessage?: (response: T) => string): Observable<T> {
    return this.pipeSnackbar(this.httpClient.post<T>(this.baseUrl + url, body), errorMessage, successMessage);
  }

  protected put<T>(url: string, body: any, errorMessage: (error: HttpErrorResponse) => string, successMessage?: (response: T) => string): Observable<T> {
